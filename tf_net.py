...
def net(input_size, flat_P, n_channels, filter_widths, pool_sizes, n_hidden, learning_rate=0.002, l2reg=0.001, fixed=[], additional_channels=0):

    x=T.ftensor4('x') # samples x channels x 1 x (4+additional channels)
    ...
    for layer_index in range(len(n_channels)):
        ...
        # input_size=(None, input_channels, 1, input_size),  is deprecated
...
def check_valid(input_length, filter_widths, pool_sizes):
    for i in range(len(filter_widths)):
        input_length = (input_length - filter_widths[i] + 1) // pool_sizes[i]
        if input_length <= 0:
            return(False)
    return(True)
